...
for i =1 : standot_num-1
    ...
    rec(5,:)=rec(1,:);
    recxia(i,1:2)=rec(1,1:2);
    recxia(i,3:4)=rec(2,1:2);
    recxia(i,5:6)=rec(3,1:2);
    recxia(i,7:8)=rec(4,1:2);
    plot(rec(:,1),rec(:,2),'g');
...
ylabel('lat');
legend('Bus Route');
title("嘉定104路公交下行矩形范围");

%standard输出
xlswrite('rectangle_xia.xlsx',recxia);
xlswrite('standard_xia.xlsx',standard);
